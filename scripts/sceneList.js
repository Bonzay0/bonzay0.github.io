function writeScene(scene) {
	...
	switch(scene) {
		...
		case "start" : {
			...
			writeText("And I'm supported by my other patrons as well. Thank you to Swallows999, HiCaTe, Daniel L Folkvord, Robbie, SlackerSavior, DesertCamel, Christopher Bartz, shadow goob82, LostSand, Eric Simpkins, Ora494, Noel Chandler Sanders Metz, K DA, Vikignir, William Thomson, Ruinn, Louts, HiCaTe, Dylan, ArtemisAisu, Onii-sama, xdrake100, Holden Martin, Matthew Landin, that GUY, Elias, Iron Diana, 4MinuteWarning, Hi There, RimuruTempest, 林家豪, Drakcon, yami, Ramsey Bisher, jack spencer, maxime, J_C_L, Jinouga, Marco Wassmer, Colin E, Badaxe, Scal, 담배잎 먹어보니, Dominic, nathan Deglman, Zee5531, Tsereth, Arkanian001, KIVA, John Greenwood, Gauthier Kreilmann, Isaiah Sandoval, Mori Jin, jimbob337, Keiteu Gaming, Neyafi, Cameron Farabee, Oz, chibiterasu100, Lukas Caspers, Nicola M, GRiiMM GAMiNG, Double Dot, GWQwQ, dylan Spindat, Nichole Cain, Venomill, Zachary Schicker, Indytronic, Andrew Corrigan, J, Oliver, Matthew Preston, Mr.Artrist, DigiReave, Johnathen Likens, Sumanth, Mango, Maxwell Dedmon, TeLoad, Ary Toussaint, faaaa, Cameron Geddes, brybry, xXSelenaXx, Spartan 5398, Boo3, Danny Surety, robert therrien, Patrik LopariÄ‡, Dylan York, Todd Vogel, A Channel, Glenn Losey, joseph, dawson, Michael Graham, Christopher Nevell, EgoDraconis, Petrichor, hunter sipper, Mariusz, Juraijin1, Traveler123, Justin, Sean sullivan, D'andre, David Outram, PIGDER, Andrew, Sgt.Vodka, Kient Wong, Harry Foster-major, Aaron Brown, Ash mash, Calla Smith, Bastl, Rafa Smith, pest, Zach allen, Carson Goodwin, Romnomnom, Dragoon22, mitchellwolbert, Wells Wells, undead270, ThatOtherGuy, Ashwinder, Bryan Pasternack, robert zeuke, omb, Hello Darkness, Regulus, Funnykitty, Patrick, Seamus Porter, BINKS, ShadowDragon, Richard Pryor, Zane regel, HolyDurandal, John Nixon, Donovan Korpa, Noh Bhodie, Billymays, Genxin, Ethan Brian, poyo, Greatsage56 ., sb2017, Wysdom, Me Notyou, john sparks, Darien Cordero, Brandon Leary, Sergio Prichard, Sera, ggboomsky, RatKnight, Bradley Herbert, SomebodyElse, Peter H, 长平 鬣, Culton Niccum, thesingleguy, Anton Schmidt, david thompson, Jayesonn leleaux, Bleed1ngthunder, Eric Hslew, Vikteren, theoron, AnotherAccount333, Kaliden, greatred, Limitless, KVBismarc, Geoff Heimos, liam paterson, ChubbiestThread, Kieron Kow, Slomberg, henry grobins, ddkre, SmolFish, Iâ€™m a Mitch, GirmusCz, Daniel Vivette, Harry Brasch, Markus Hansen, Leanerbike1363, Tanman, Erin Trippet, Danny Nguyen, Grim2011, Darrell Goodman, Bonelessunknown, Phanes, Scumstango, Aegil, Wei, Dugelle, Stollwerk, Chaaaanon, Nutburger, Noah675, mazterlith, john smith, Your Husbando, Carlos, Verillia tristari, ShadowHunter1394, ItsAllOgreNow, Jesse Greene, Nils Maier, Ariados, Jane, Snaked, qwerty, murgatroid99, Roy, Wild Bill, ChronosEdge, and brandon for supporting my work!");
			...
			switch (data.player.day) {
				...
				default: {
					if (data.player.time != "Night") {
						...
						if (data.player.color.includes('introComplete') == true && data.player.color.includes('beta') != true) {
							writeSpeech("assistant", "", "Hey, agentL wanted to talk to you. It seems like you've got another field mission ready.");
							writeTransition("beta", "Head to agentL's office.");
						}
						if (data.player.color.includes('betaComplete') == true && data.player.color.includes('finale') != true) {
							writeSpeech("assistant", "", "Hey, agentL wanted to talk to you. It seems like you've got another field mission ready.");
							writeTransition("finale", "Head to agentL's office.");
						}
					}
					switch (data.player.time) {
						case "Morning": {
		...
		case "vault": {
			if (data.player.color.includes('vault') != true) {
				...
				if (data.player.time == "Night") {
					...
					writeArtifact("vhs");
					writeArtifact("doll");
					if (data.player.color.includes("charity")) {
						writeArtifact("charity");
					}
					if (data.player.color.includes("handMirror")) {
						writeArtifact("handMirror");
					}
		...
		case "beta": {
			data.player.color += "beta";
			writeHTML(`
				player Oof, alright. Time for another trek.
				t ...
				t You give agentL's door a light rap, your legs are sore but you're nowhere near as tired as you were the last time. After a knock and an invitation you step inside.
				agent Heya! How's the vault's freshest agent doing?
				player I'm alright. Is that trip gonna be this exhausting every time?
				agent Nice! And yeah, it's actually a permanent thing, but you'll get used to it. Glad you're improving though. Anyways, here.
				t agentF tosses you several papers, the first of which is a a small scrap with the word 'beta' on it.
				agent Personally I think this one's a little above your experience level. Take care, yeah?
				player Where am I headed?
				agent A vault's gone kaput, a full breach. Top brass wants you to recover the chairwoman inside.<br>I'm serious, be careful. The place is a total blacksite, their entire prison department could be loose inside for all we know.
				player That really doesn't sound like a one man job...
				agent Yeah, especially for a newbie, but I asked and they said I'm not allowed in there for some reason.<br>Luckily when a vault goes on lockdown everything's sealed up tight, so if all goes well it'll be an in-and-out job. Plus it's been a few days, and most of the other staff actually managed to escape into our custody, so it can't be too bad in there. Go see Charlie in the storage department to get started.
			`);
			writeTransition("office", "Finish");
			break;
		}
		case "finale": {
			data.player.color += "finale";
			writeHTML(`
				t After a short trip you wind up at agentL's office once again, barely even having broken a sweat. After a knock and an invitation you step inside.
				agent Welcome back!
				player Another mission already? What's up, need me to head back inside the beta vault? Explore the lost depths of Atlantis?
				agent Close! We need you and your assistant to move a cart down a few hallways.
				t There's a moment of silence...
				agent Hah! The look on your face!<br>Hey, don't look so glum, not every mission will take you across the country.
				t She hands you a few papers. The first of which is a scrap of paper labelled 'finale'.
				agent We've finally finished securing another artifact, this one might be a little familiar. 
				im scripts/gamefiles/items/doves.jpg
				player The lovey doves?
				agent Yep! You'll be wheeling them down to the dark vault. I Think this puts the vault at full capacity of high-risk artifacts, actually.
				player I don't know, the last time I was around these-
				agent Don't worry. Research shows they're a lot less influential over people they've already interfaced with. Plus we have them under psychic shielding. You and your assistant just need to handle the delivery. Two people in case we need one to take the other out.
				t She says it so casually...
				agent Most daily goings-on have been put on hold for today, so no need to worry about bumping into anyone. Me and bossL will be making sure nothing else decides to throw a tantrum just in case. If something goes wrong it's best to make sure nothing else cascade, you know? Anyways, go see Charlie in the storage department to get started.
				player You had me walk all the way over here, just to walk to the storage department, just to walk back to transport the doves?
				agent Yep! Great cardio, huh?
			`);
			writeTransition("office", "Finish");
			break;
		}
		...
		case "mirrorResearch": {
			...
			var mirrorUnlocks = 0;
			var specialScene = "";
			if (data.player.color.includes("mirrorPrompt1") != true) {
				for (aa = 0; aa < galleryArray.length; aa++) {
					if (galleryArray[aa].index.includes('mirrorResearch1')) {
						mirrorTotal += 1;
						if (galleryCheck(galleryArray[aa].index) == true) {
							mirrorUnlocks += 1;
						}
					}
				}
				...
				if (mirrorUnlocks == mirrorTotal) {
					specialScene = "mirrorPrompt1";
				}
			}
			mirrorTotal = 0;
			mirrorUnlocks = 0;
			if (data.player.color.includes("mirrorPrompt2") != true) {
				for (aa = 0; aa < galleryArray.length; aa++) {
					if (galleryArray[aa].index.includes('mirrorResearch2')) {
						mirrorTotal += 1;
						if (galleryCheck(galleryArray[aa].index) == true) {
							mirrorUnlocks += 1;
						}
				...
				console.log("mirror total is " +mirrorTotal);
				if (mirrorUnlocks == mirrorTotal) {
					specialScene = "mirrorPrompt2";
				}
			}
			if (specialScene != "") {
				sceneTransition(specialScene);
			}
			break;
		}
		case "mirrorResearch1": {
			...
			if (galleryCheck('mirrorResearch1-1') == true) {
				...
				if (galleryCheck('mirrorResearch1-6') == false) {
					writeFunction("writeEvent('mirrorResearch1-6')", "Participate in artifact research");
				}
				var mirrorTotal = 0;
				var mirrorUnlocks = 0;
				for (a = 0; a < galleryArray.length; a++) {
					if (galleryArray[a].index.includes('mirrorResearch1')) {
						mirrorTotal += 1;
						if (galleryCheck(galleryArray[a].index) == true) {
							mirrorUnlocks += 1;
						}
					}
				}
				...
				console.log("mirror total is " +mirrorTotal);
				if (mirrorUnlocks == mirrorTotal) {
					writeFunction("writeEvent('mirrorEnding1')", "Ask about a more permanent position here");
				}
		...
		case "mirrorFailed1": {
			...
			writeText("That might not be a bad thing, but it's a big decision you should leave for later.");
		...
		case "mirrorPrompt1": {
			writeHTML(`
				t You step up to the mirror as normal taking a glance at the Little Men are Fun Toys dimension, only to notice that your reflection is wearing a really nice-looking suit. He seems pretty preoccupied checking his outfit.
				player Huh. Any idea what he's doing?
				assistant Yeah, he's been dating this one girl in the city. <span style = "size:50%;">Lucky bitch...</span>
				player What was that?
				assistant Nothing!
				t He actually looks pretty psyched. It's interesting to know what he's been up to while you've been messing around in his dimension. Maybe you should give some thought to a lengthier stay?
			`);
			data.player.color += "mirrorPrompt1";
			writeTransition("mirrorResearch", "Continue");
			break;
		}
		case "mirrorResearch2": {
			...
			if (galleryCheck('mirrorResearch2-1') == true) {
				...
				console.log("mirror total is " +mirrorTotal);
				if (mirrorUnlocks == mirrorTotal) {
					writeFunction("writeEvent('mirrorEnding2')", "Ask about a longer stay here");
				}
				writeTransition("work", "Head back to your dimension");
			}
			else {
		...
		case "mirrorPrompt2": {
			writeHTML(`
				t You step up to the mirror as normal taking a glance at the Public Fun Is Not Unusual dimension, only to notice that your reflection is a lot more enthusiastic than normal.
				player ... Hey, what's my doppelganger been up to while I'm away?
				assistant He's, uh... A bit eccentric. Apparently he's been hanging out around the seedier parts of the city hitting on women.<br>As far as I can tell he's super into how much clothes people wear over here, and how 'forbidden' sex is.
				player Interesting. I wonder how he'd feel if he had a week or something over here, instead of just an hour.
				assistant Well, long-term cross-dimension stuff is something we've been meaning to research. Just don't come back <i>totally</i> sex-addled, okay?
				player Me? Never!
			`);
			data.player.color += "mirrorPrompt2";
			writeTransition("mirrorResearch", "Continue");
			break;
		}
		...
		case "stopwatchFailed": {
			...
			if (scenes[0] == scenes[1]) {
				...
				writeHTML(`
					...
					t Your inquisitive nature seeks answers, but... Something about the idea fills you with a dream for some reason. Just what <i>is</i> this thing? Why does it even tick at all?
		...
		case "erotiboxResearch": {
			...
			if (galleryCheck('erotiboxResearch17') != true && data.player.color.includes("shawl")) {
				writeFunction("writeEvent('erotiboxResearch17')", "Test the madameF's shawl");
			}
			if (data.player.color.includes("miscbox") == true) {
		...
		case "perfumeResearch": {
			tempScene = 'work';
			if (galleryCheck('perfumeResearch1') == true) {
				writeSpeech("player", "", "A completely sentient artifact... I wonder how I should actually go about researching it.");
				if (galleryCheck("perfumeResearch2") != true) {
					writeFunction("writeEvent('perfumeResearch2')", "Let the madameF interact with assistantF.");
				}
				if (galleryCheck("perfumeResearch3") != true) {
					writeFunction("writeEvent('perfumeResearch3')", "Let the madameF interact with bossL.");
				}
				writeTransition("perfumeTalk", "Just chat.");
				writeTransition("work", "Go back");
			}
			if (galleryCheck('perfumeResearch1') != true) {
				data.story[5].fName = "Madame";
				data.story[5].lName = "Sunny Leone";
				writeEvent('perfumeResearch1');
				data.player.perfumeHappy = 0;
			}
			break;
		}
		case "perfumeTalk": {
			tempScene = 'work';
			writeHTML(`
				player Well, there's not much to do here. 
				madame Indeed. Why not bring me with you outside? I find sunlight accentuates my features much better than concrete walls.
				player Maybe. But while we're here, let's chat. 
			`);
			switch(data.player.perfumeHappy) {
				case 0:
					writeText("The madameF seems annoyed at this, and doesn't even bother sitting down.");
					writeSpeech("madame", "", "And why exactly should I? I spoke plenty during our first meeting, so I don't feel like talking any more.");
				break;
				case 1:
					writeText("The madameF seems annoyed at this, but begrudgingly sits down.");
					writeSpeech("madame", "", "If you intend to dangle only the smallest of rewards, don't expect anything of great value.");
					writeFunction("sceneTransition('perfumeTalkAge')", "Talk about her, where and when she comes from.");
					writeText("She doesn't like you enough to speak of her true nature.");
					writeText("She doesn't like you enough to speak of other artifacts.");
					writeText("She doesn't like you enough to speak of her sexuality.");
					writeText("She doesn't like you enough to speak of her possessions.");
				break;
				case 2:
					writeText("The madameF sighs, but sits down anyways.");
					writeSpeech("madame", "", "Fine, what is it? I'd like to see the sun again at least before your next interrogation.");
					writeFunction("sceneTransition('perfumeTalkAge')", "Talk about her, where and when she comes from.");
					writeFunction("sceneTransition('perfumeTalkNature')", "Talk about true nature. What exactly is she?");
					writeText("She doesn't like you enough to speak of other artifacts.");
					writeText("She doesn't like you enough to speak of her sexuality.");
					writeText("She doesn't like you enough to speak of her possessions.");
				break;
				case 3:
					writeText("The madameF decides this is agreeable, and takes a seat.");
					writeSpeech("madame", "", "Very well then, I suppose you've more than held your end of the bargain.");
					writeFunction("sceneTransition('perfumeTalkAge')", "Talk about her, where and when she comes from.");
					writeFunction("sceneTransition('perfumeTalkNature')", "Talk about true nature. What exactly is she?");
					writeFunction("sceneTransition('perfumeTalkArtifacts')", "Talk about artifacts, and if she knows the location of any.");
					writeText("She doesn't like you enough to speak of her sexuality.");
					writeText("She doesn't like you enough to speak of her possessions.");
				break;
				case 4:
					writeText("The madameF decides this is agreeable, and takes a seat.");
					writeSpeech("madame", "", "Very well then scholar, ask away.");
					writeFunction("sceneTransition('perfumeTalkAge')", "Talk about her, where and when she comes from.");
					writeFunction("sceneTransition('perfumeTalkNature')", "Talk about true nature. What exactly is she?");
					writeFunction("sceneTransition('perfumeTalkArtifacts')", "Talk about artifacts, and if she knows the location of any.");
					writeFunction("sceneTransition('perfumeTalkSexuality')", "Talk about her sexuality and preferences.");
					writeText("She doesn't like you enough to speak of her possessions.");
				break;
				default:
					writeText("The madameF decides this is agreeable, and takes a seat.");
					...
					if (data.player.color.includes("shawl") != true) {
						writeFunction("sceneTransition('perfumeTalkShawl')", "Talk about her possessions.");
					}
					if (galleryCheck("erotiboxResearch17") == true) {
						writeFunction("sceneTransition('perfumeTalkResult')", "Talk about her shawl, and how it was transformed.");
					}
					writeText("The madameF seems quite content with her place here. It seems you've satisfied her desires for now, and she probably won't be keeping any secrets from you any more.");
			}
			writeFunction("sceneTransition('work')", "Finish talking.");
			break;
		}
		case "perfumeTalkAge": {
			writeHTML(`
				player So, can you tell me about yourself? Where you come from, or how old you are?
				madame Oho, in my time it was rude to ask a woman her age. Well, I suppose times have changed.<br>I do not know how much time has passed, only that the world has changed and the stars are in different positions. I only re-awoke a few years ago, and have been stuck in these drab grey boxes for most of them.
				player I see, I wonder if you even age at all. You could serve as a recordkeeper in case of an end-of-the-world scenario...
				madame You're quite fastidious at times, scholar.
				player Hey, I'm a seasoned researcher here. I can get serious when I need to.
			`);
			writeTransition("perfumeTalk", "Talk about something else");
			writeTransition("work", "Finish talking");
			break;
		}
		case "perfumeTalkNature": {
			writeHTML(`
				player So, what are you?
				madame You'd ask a lady for her greatest piece of leverage? I quite enjoy our dalliances together.
				player So do I, so give me a reason to continue with them And maybe go even further. The beans, spill.
				t She looks baffled for a moment, but shrugs it off.
				madame I am a Jinn, a natural being.
				player Oh, like a genie? So the bottle is your lamp?
				madame I was bound to the bottle by an old king, I was commanded to aid humanity achieve greatness and wisdom.<br>That said, the knowledge needed to control me has almost certainly been lost to history, and the plan almost certainly backfired.
				player How can you be sure?
				madame The old king had a specific definition of 'achieve greatness' in mind, and the fact that we're speaking English right now tells me he didn't achieve his goals.
				player Right. English. How did you learn it?
				madame My kind are quick to learn. Very quick.
				player I see. Are your kind normally hostile to humans?
				madame Do not compare us to shayatin, scholar, that's quite rude. No, we'd rather have nothing to do with you at all.<br>Well, I assume. I haven't seen another of my kind since my binding.
				player I see. Well, any specific bit of wisdom to end out chat on?
				madame Sex is more enjoyable than war.
			`);
			writeTransition("perfumeTalk", "Talk about something else");
			writeTransition("work", "Finish talking");
			break;
		}
		case "perfumeTalkArtifacts": {
			writeHTML(`
				player Are you connected to any other artifacts? Anything you could help the vault find and secure?
				t She looks thoughtful for a moment.
				madame My vessel was discovered once before, by another king. Less impressive than the one who bound me. He was... Greek, I think, and considered himself a collector of oddities.
				player I see. Is he still alive? Where could we find him, or his remains?
				madame He's here, in the vault, actually. I saw us pass him by in another room. The fool got himself sealed into a pauper's coin by some demigod he'd offended.
				player The midas coin?
				madame Be careful with that trinket, his spirit is still inside of the thing. Well, we were close, so I suppose he'd free me from this prison if his were broken... <br>With that in mind, feel free to play as roughly with the coin as you please.
			`);
			writeTransition("perfumeTalk", "Talk about something else");
			writeTransition("work", "Finish talking");
			break;
		}
		case "perfumeTalkSexuality": {
			writeHTML(`
				player So, you prefer the company of women?
				madame I prefer the <i>everything</i> of women, scholar. The softness of their skin, of their voice, how pliable and lovely they feel beneath my hands~
				player Right. Is that because of your species, your nature, personal preference?
				madame It's a matter of taste. I do find certain aspects of the other sex enjoyable at times. I could even see you as something to enjoy if I were particularly bored. And you weren't, well... Quite so...
				t She vaguely gestures at you.
				player ... Right. Well, for the sake of science I should introduce you to a femboy or something eventually. Still, I think I've gotten everything I need from this.
				madame Good, I'm glad we can come to an understanding. One of the previous carers for my vessel was much less, well...<br>He threw the bottle into the desert, and I found the sands to be much better company. A shame about his wife though.
			`);
			writeTransition("perfumeTalk", "Talk about something else");
			writeTransition("work", "Finish talking");
			break;
		}
		case "perfumeTalkShawl": {
			writeHTML(`
				player You seem to have the ability to pull things from nowhere. Those clothes for example. Are they real?
				madame My clothes are as real as yours are. Actually, here. I saw us pass by something entertaining. In one of the rooms was a wooden box that seemed to have a number of interesting abilities.
				t She pulls a shawl from thin air and hands it to you.
				player You want me to put this in the erotibox? I don't see why not, where did it come from?
				madame It was a personal possession of the old king who bound me. I've kept it hoping I'd find an enjoyable way of defiling it some day. Do please let me know what becomes of the rag.
			`);
			data.player.color += "shawl";
			writeTransition("perfumeTalk", "Talk about something else");
			writeTransition("work", "Finish talking");
			break;
		}
		case "perfumeTalkResult": {
			writeHTML(`
				player I've got your shawl back, here.
				t You return the madameF's shawl to her.
				madame Oh, how delightful. Let's see...<br>Hmm, the religious text is gone. I see.
				player Yeah I don't think the box understands stuff like that.
				madame Hmm... A shame. Not about the rag, I mean.<br>My vessel is actually part of a trio, crafted by a woman of eccentric tastes. She would never have removed the text this way, I suppose she really is gone...
				player A trio?
				madame Yes. Three artifacts. One to perceive the hidden, one to become hidden, and one to ensure no one wishes to find you even if they cold. When combined...<br>This place seems quite serious about threats, am I correct?
				player Yeah. Dangerous things tend to get locked up, and the keyholes welded shut.
				madame Then I shan't speak more, for your sake. Just knowing about the trio would likely direct an unfortunate amount of attention to you.
			`);
			writeTransition("perfumeTalk", "Talk about something else");
			writeTransition("work", "Finish talking");
			break;
		...
		case "charityResearch": {
			tempScene = 'work';
			researchLevel('charity');
			break;
		}
		case "charityFailed": {
			tempScene = "work";
			writeEvent('charityResearch2');
			break;
		}
		case "handMirrorResearch": {
			tempScene = 'work';
			writeFunction("writeEvent('handMirrorResearch1')", "Observe roommateF's fate");
			if (data.player.color.includes("katyaIntro")) {
			writeFunction("writeEvent('handMirrorResearch2')", "Observe sisterF's fate");
			}
			writeFunction("writeEvent('handMirrorResearch3')", "Observe bossF's fate");
			writeTransition("work", "Resist the temptation and walk away");
			break;
		}
		case "doctorResearch": {
		...
		case "missionSelect": {
			...
			`;
			if (data.player.color.includes('interview') == true) {
				writeText("You've unlocked the mission codenamed <b>intro</b>");
			}
			if (data.player.color.includes('beta') == true) {
				writeText("You've unlocked the mission codenamed <b>beta</b>");
			}
			if (data.player.color.includes('finale') == true) {
				writeText("You've unlocked the mission codenamed <b>finale</b>");
			}
			writeTransition("storage", "Go back");
			break;
		}
	...
	switch (scene) {
		...
		case "braceletLibrary3": {
			writeHTML(`
				...
				t In any case, it might be interesting to play with her more though, are there any other artifacts to further explore this line of thought?
			`);
		...
		case "charmOutdoor1": {
			writeHTML(`
				...
				sp ???; im images/none.png; Oh god, it's so thick! No matter how much I swallow the taste won't go away! 
				...
				... 
				sp ???; im images/none.png; I can't... No more... 
		...
		case "charmLibrary1": {
			writeHTML(`
				...
				t Yet as she clearly sees herself shudder in orgasm, her hopes are dashed. Anyone seeing this would just assume it's some roleplay porn featuring a librarian getting off on public sex...
		...
		case "erotiboxResearch17": { //wand
			writeHTML(`
				sp player; Now beginning test EB-17, I've got a shawl from the madameF.
				assistant You trust her enough to put that inside the box? I thought you told me to take her word with a grain of salt.
				player Hey, worst case scenario this turns out badly, and we get insight into her true intentions.
				assistant And the best case scenario?
				player We do science. 
				t You place the shawl in the box, then shut the front gate. 
				t Within seconds a bright purple light illuminates the room, before quickly dying down again.
				t Inside the box is...
				t ... A black scarf?
				assistant Just a scarf?
				player That's not even the same type of clothing. Bad box.
				t The box does not respond to your scolding.
				player ... Right, so, wear it.
				assistant Why am I always...
				t assistantF grumbles to herself as she picks up the scarf and wraps it around her. After a few moments she looks a little flush.
				player So, what's it doing?
				assistant N-nothing! It's just really hot in here, and I'm wearing a scarf! It seems like this one's a dud.
				t ...
				t After a while longer it really does seem like nothing's happening. But you only took your eyes off her for a second to log something before you heard a soft squelch coming from assistantF.
				t But as you turn around there's no sign of what made the sound, aside from assistantF looking away from you shyly.
				t ...
				t Following your instincts you allow assistantF to go about her day wearing the scarf, all the while you follow unnoticed behind her. Only a minute after you allowed her to leave though she made a beeline for her desk to find...
				im images/box/erotiboxResearch17.gif
				assistant Hehe... Ehe... This is so wrong~
				player <i>Alright, this is a bit much, even for her.</i>
				sp notes; Findings: The madameF's shawl was replaced with a black scarf which, when worn, would cause exhibitionist tendencies within the subject to flare up rapidly as well as potentially causing an increase in the subject's percieved temperature.<br>This results in an escalating desire for high-risk self-pleasure, the goal or upper bounds of which are currently unknown.
			`);
			break;
		}
		...
		case "erotiboxMiscWand3": {
			writeHTML(`
				...
				t Some kind of dance for social media it seems. Well, this ought to be a fun use of the wand. You set the target setting to 'UNAWARE' and leave the audience setting on 'AWARE'.
				...
				t You click the target setting to 'AWARE' and hit the button again, and struggle to hold back laughter as sisterF's eyes shoot open as she sees herself as her audience sees her.
				sp sister; I... I...
		...
		case "mirrorResearch1-6": {
			writeHTML(`
				sp assistant; Huh. Well, we're not going to turn away a willing test subject, especially one that's already familiar with the the Vault.
				sp player; Great. Do you have a list of artifacts for me to pick from?
				t assistantF chuckles to herself, shaking her head.
				sp assistant; Maybe it's different in your world honey, but around here, the test subjects don't get to control the tests.
				t You tense up a bit at that, but all things considered, it's not suprising...
				t Still, they probably won't put you through anything too bad.
				...
				im images/mirror/research1-6-1.gif
				sp player; Mmph...!
				t Your moans are stifled through a ball-gag as assistantF's tongue teases at your shaft, and the handcuffs (soft and fuzzy as they are) kept you from moving much.
				sp assistant; Mm... As expected, the lipstick has a powerful effect on oral pleasure~
				t Her fingers continue to gently tease your shaft as she kisses your tip again, causing your entire body to shudder in electric ecstasy as she does.
				sp assistant; It seems the name 'Sensuality Concentrate' was an accurate one, if those muffled moans are anything to go by.
				im images/mirror/research1-6-2.gif
				t The cloth on her hands isn't an artifact or anything, but the soft and smooth texture only adds to the sensation of being blown and jerked off.
				t Even the parts of the shaft she hasn't touched with her lips feel insanely sensitive, but the small streaks of red left behind are almost overwhelming you as your cock spasms in her mouth.
				t You're not sure how long she's kept you on the knife's edge of cumming, but you can easily tell the moment she decides to stop holding back when she smirks up at you.
				sp assistant; That expression of yours is absolutely adorable, playerF.
				t With that, she purses her lips tightly and takes you balls-deep all at once, her lipstick brushing against every inch of your shaft as your vision starts to go white.
				t By the time she pulls her head back up, your cock is already about to burst.
				im images/mirror/research1-6-3.gif
				t Your mind goes blank as you watch your cum spurt out onto her beautiful ruby lips, her hand milking your cock skillfully with small motions as your entire body shakes.
				t When your head finally clears, you can see assistantF carefully wiping at her lips with a damp cloth, and the ball-gag is on the table beside you. The cuffs are loosened, too, so it's not hard to step down.
				sp player; Th-That was... intense.
				sp assistant; I'll say. You were limp and panting there for thirty seconds before you just came to.
				t You roll your shoulders a bit, curiously eyeing the 'Sensuality Concentrate'.
				sp player; I think it has some kind of mental effect too. Finishing on your lips was... <i>extremely</i> satisfying.
				t She quirks an eyebrow, before smiling and nodding once.
				sp assistant; I'll have that noted in the log, thank you.
				t Her hand ruffles at your hair as she smirks.
				sp assistant; Good boy~
				t With that, she winks at you once and strides out of the room, clipboard in hand as she examines the log... and, you notice, subconsciously licks her lips.
			`)
			writeSpecial("This scene was written by <span class = 'switch' onclick='window.location.href=`https://www.reddit.com/user/CaptainCryptogreek`'>Captain Cryptogreek</span>");
			if (tempScene == 'work') {
				tempScene = 'mirrorResearch';
		...
		case "mirrorEnding1": {
			writeHTML(`
				assistant What, like working as a permanent test subject?
				player Actually, I was thinking as a higher-level researcher, or field agent. I've got the experience.
				assistant An agent from another dimension?<br>... Well, I don't know. bossL's in charge of that, I guess we can go ask her.
				t The two of you head out of the testing lab over to bossL's office. Practically the moment you bring up the subject of working here...
				boss Hired. Report downstairs, you can have playerF's locker.
				player Just like that?
				boss You're already fully trained and you're immune to the majority of the things we'd throw in the dark vault. Plus playerF's been performing poorly lately, we were already considering canning him. You'll be quarantined for a bit and exposed to a few objects to see if you being from another dimension causes any problems. Other than that, I don't see an issue.
				t ...
				t After a pretty boring week of health and knowledge testing, you're officially taken on as a field agent in this dimension's vault.
				t You head on down to the testing lab. At some point you earned the title of 'finest piece of ass in the vault', and the poorly-masked stares you get on your way show the women on this floor know how you earned it. You approach the office of your partner, agentF, only to hear she's already talking to someone.
				assistant I can't believe they assigned playerF as your partner.
				agent Hey, what can I say? Maybe they wanted to throw me a bone?
				assistant Hah! Spoken like a total virgin.
				agent It'll be fine. I'll come on slow, I don't want him to think I'm some kind of creeper.
				assistant You better. If you make me sit through another misconduct training meeting I'll-
				t She opens the door, finally noticing you.
				assistant Oh, hey, speak of the devil.
				agent Heeeey~! Well, if it isn't my new partner. You're right on time. I just got the details, we're to call a ride and get going asap.<br>You'll try not to slow me down, yeah?
				t The atmosphere here is overall pretty friendly, although to be honest it can be a bit patronizing at times.
				t ... Or maybe matronizing is the word?
				agent Hello~? Earth to playerF! Heh, you nervous? No worries, I'll show you the ropes, just try to stay out of trouble.
				t ...
				t It's been a few months since you've joined this dimension's vault as an agent. You haven't been pulled back through, so your doppleganger must be having a pretty good time so far.
				t And with just a few missions under your belt you're already meeting with chairF! That said, the subject of your meeting is a bit...
				im images/mirror/endingA-11.gif
				im images/mirror/endingA-12.gif
				agent Let me out! LET ME OUT! I NEED TO <b>BREED</b>!
				chair So. How exactly did this happen?
				player The artifact we were investigating turned out to be something called the Domina PMV. Some kind of bimbofication effect, the effects aren't too pronounced except for repeat viewings.
				chair And how many times did agentF watch it?
				player At least seventy. She was also listening to just the audio through headphones. Apparently she caught a glimpse of it while we were separated and got hooked instantly.
				chair Hah... And of course it only effects women. I swear it feels like everything in the vault only goes after the fairer sex. No offense.
				player None taken, I get what you mean. <br>In my dimension a lot of these have a different effect. At least some seem to be neutral towards gender, like the box.
				chair Don't get me started on the box! When I heard that assistantF had thrown you inside for the sake of science, or that she'd let you out because she'd been blinded by lust...
				im images/mirror/endingA-2.gif
				chair I'm still trying to balance the maternity leave budget. Did you really need to impregnate half the staff?
				player Sorry. Why were there so many female staff members on duty that day anyways?
				chair Don't ask.<br>Anyways, until we can fix agentL, you'll need a new partner...<br>Your former supervisor, bossL, what happened to her?
				player Ah, it turned out she'd been abusing her dark vault privileges.
				im images/mirror/endingA-3.gif
				player There's an item down there called the Incubus Dildo. She snuck down and-
				chair I know what it does. God damn it... How far along is she?
				player Two weeks, she's due any day now.
				chair Hah... We need more competent staff, or at least someone to train them better.<br>... playerF?
				player Yeah?
				chair I think it's about time you got a promotion. 
			`);
			break;
		}
		...
		case "mirrorEnding2": {
			...
			writeHTML(`
				assistant You're looking for something to keep you occupied for a while? Not a bad idea, some of the staff have been a bit curious about how you'll do over here.<br>Actually, I think I've got just the place in mind.
				t ...
				define ? = sp ???; im images/none.png;
				? Welcome back everyone to day 4! I know you're all racing to know how our contestants are doing, so without further ado...
				im images/mirror/endingB-1.gif
				? Our dark horse playerF is still going strong, currently holding the lead by a wide margin. Having only needed three breaks so far, he could very well be on his way to setting a new city record!
				t The cheer of the audience is almost deafening, <i>almost</i> able to drown out the sound of all these women trying to drain your balls as efficiently as humanly possible.
				t For everything you've seen in this dimension, the fact that <i>this</i> is what modern sports look like is something you have a hard time wrapping your head around.
				im images/mirror/endingB-2.gif
				t You still aren't totally clear on the rules, but the gist you've gotten so far is to just keep going for as long as possible. You're given time to eat and sleep, but aside from that it's been hour after hour, day after day doing what you do best.
				assistant Wooo~! Keep going playerF~!
				im images/mirror/endingB-3.gif
				? Could he? I think... I think he could! I think we have a winner folks, and a new record at that! My god, could he be going for a world record next? Are we witnessing history in the making?!
				t It seems like fame awaits you, and given that you haven't been pulled back yet it seems your doppelganger is enjoying himself too. Maybe, just maybe...
				t You might stick around here a while longer.
			`);
			if (testThing == true) {
				data.player.pervertLevel = 0;
			}
		...
		case "perfumeResearch1": {
			writeHTML(`
				assistant Alright, this time we've got a bottle of perfume. Apparently this was found in a desert on the other side of the world.
				player Crazy how it made its way all the way over here.
				assistant Right? We've also got the old vault's file on it from before it came here. It was found by a trader, who got assasinated and robbed by a merchant. Then it got traded to a local prince, and-
				t You interupt assistantF with a light application of the perfume.
				assistant Gah, you jerk! Huh, it does actually kind of smell-
				t Suddenly assistantF falls backwards, dropping her clipboard and pointing to an empty corner of the room in horror.
				t You decide to take a sniff, and where there once was nothing now stands...
				im scripts/gamefiles/characters/madame.jpg
				madame Greetings. Are you the new owner of my vessel? I'd prefer it to be the girl behind you, but you seem the one more in charge.
				t ...
				madame No, I do not grant wishes.
				assistant Are you a ghost?!
				t After she got over her initial shock, assistantF mostly took charge of the questioning while you go through the files the previous vault had on this entiry known as the madameF.
				player Let's see... Low danger... Limited to the vicinity around the bottle... Able to influence others...<br>Hey, madameF, what's the limit on this suggestion ability?
				madame It's merely that I can sound like one's own thoughts, so long as whoever I'm speaking to isn't aware of me.
				assistant This is the first time we've researched a talking artifact. This is a lot easier!
				player Take it with a grain of salt, assistantF. She likely has her own goals, and could be lying to us.
				assistant Aww, why would she do that? Hey, miss madameF, you know anything about yourself that isn't in the old files?
				madame Why yes, actually.
				t The madameF takes assistantF's chin in her hand and leans in face-to-face.
				madame I find women like yourself... Absolutely irresistible.
				sp notes; Findings: <br>When a subject is exposed to the oil contained within the artifact, they become aware of a humanoid female entity known as the madameF. While the entity refers to the bottle as her 'vessel', there is no clear link between the two.<br>The madameF appears only able to influence objects within the immediate vicinity of her vessel. In addition while she is also able to make suggestions and commands to others, these are ineffective if the person she's interacting with is aware of her presence.<br>Finds assistantF attractive.
				assistant W... What are you writing down, playerF?! 
			`);
			break;
		}
		case "perfumeResearch2": {
			writeHTML(`
				t assistantF's breaths are slow and heavy as she lays face down on her desk. 
				im images/perfume/perfumeResearch2.gif
				madame Let it take you, child. Allow your flesh to think for you, and take these sensations in stride.
				t It seems that given free reign she has about the same ideas of how to have fun you do.
				player You enjoying yourself?
				madame I am. She is a lovely work of art.
				t It seems like madameF is a little satisfied.
				madame Although I find myself missing my old playthings. They were less aware of my nature, and were less resolved to simply endure my technique. To... what is the expression? 'Grin and bear it?'
				player So you want to play with people who aren't aware of you?
				madame I want to <i>live</i>, scholar. Some place outside of a metal box, somewhere less reminiscent of my vessel.<br>That shouldn't be too difficult for one wearing your bangle, yes?
				t It seems like she's becoming bored of the locale.
				assistant Ngg... Are you... Done yet? 
			`);
			data.player.perfumeHappy += 1;
			break;
		}
		case "perfumeResearch3": {
			writeHTML(`
				im images/perfume/perfumeResearch3-1.gif
				boss Stop teasing... Fff... And...
				madame Silence, haremette. I am indulging myself.
				t Well, this escalated quickly. When you ran the idea by bossL to have her and the madameF interact, she accepted the proposal pretty quickly. She's a professional, of course.
				t She did request privacy during the testing. You ignored the request, obviously.
				boss Ghh~
				madame You see? I do not race, I allow my fruit to ripen at the pace which best suits it. 
				t Far from her usual demeanor, bossL is finding herself soft and pliable beneath the madameF's touch.
				madame You are quite well developed though. I do suppose it is time to further the education of your flesh. Do be sure to enjoy and learn in equal measure. 
				boss What are y... You even-
				im images/perfume/perfumeResearch3-2.gif
				boss Ough!!!
				t A flurry of lewd sounds and whorish gasps soon come to fill the room. bossL's well-toned body quivers like a shaken puppet beneath the madameF's masterful hands.
				t ...
				madame Hah... That such lovely flowers can grow in so cold and drab a place of learning.
				t The madame seems a little more satisfied!
				player Like a flower growing in pavement. Well, we all accepted jobs here, so I guess 'planted in pavement' would fit better.
				madame Yes. Now, I grow tired of dancing with partners. I desire clay to shape, these hands of mine should not be allowed to become stagnant. 
			`);
			data.player.perfumeHappy += 1;
			break;
		}
		case "perfumeHome1": {
			writeHTML(`
				t You opt to take the madameF on a short tour around the house. She's distracted a few times in things like your refridgerator but loses interest quickly enough. It seems she's not all that curious about modern science.
				t But her attention is taken and held by who she finds napping on the couch. It seems like girlfriendF has decided to take a rest here again.
				madame This one.
				player Are you sure? I've found she's tough to crack.
				madame Have no fear, scholar. I'm very confident in my technique.
				t ....
				im images/perfume/perfumeHome1-1.gif
				t Still asleep, girlfriendF's body is subject to an unrelenting assault by a master's touch. Naked and so covered with sweat that it creates a fine sheen, all she can do is lay and be treated as dough.
				t Yet meanwhile the madameF is the embodiment of grace. Her patient expression seems like it'd befit a master sculptor or musician more than it would someone attempting to crack grilfriendF's mind in half with pleasure.
				madame Mmm... Sleep well. Oftentimes the fantasies one concocts do much of the work. Imagination is half of the artist's pallete, after all.
				girlfriend ... roommateF...
				t girlfriendF's half-asleep mumblings are barely heard through the pillow her face is buried in, yet the madameF hears them all the same.
				madame Oh? So your thoughts drift to your lover... I shall say, scholar, I can see why you found such issue. I daresay I would have ended the sanity of a pure maiden already. It seems as though her resistance against my touch is unending. Still...
				im images/perfume/perfumeHome1-2.gif
				girlfriend Mmm...
				madame In a battle of patience I shall not be detered...
				t ...
				t In the end the madameF finally decides her work is done. girlfriendF is still asleep, although she's gone from looking like she's pleasantly napping to looking like she just endured a death valley heatwave. The couch is, well, it looks like it got hit with a pressure washer.
				t You spare a glance towards the madameF, wiping her hands clean with a fine towel she's produced from nowhere.
				madame Hmm? Spare me the look, scholar. I have ruined many an empress's linens, your cheap lounging throne is merely a sacrifice for fine art. Very fine art.
				t The madame seems a little more satisfied!
				madame Ah, the name she spoke. I'd very much like to meet the owner. 
			`);
			data.player.perfumeHappy += 1;
			break;
		}
		case "perfumeHome2": {
			writeHTML(`
				t You take a step into roommateF's room, using the bracelet to keep yourself unnoticed. It seems like she's just gotten home from work, and as she's getting changed madameF wastes no time laying her hands on roommateF's body.
				madame Lovely...
				t roommateF shudders against the sudden phantom sensations of molestation, before the madameF materializes a small bottle in her hand. Seemingly a bottle of the same liquid you used to become aware of her. madameF sprays a light mist upon roommateF's body.
				madame I must say, I'm stricken. 
				roommate What?! W-who-
				madame Count yourself lucky, my dear. Often I prefer to keep myself hidden to my playthings, but you are special.
				roommate Let me g- Ough!
				madame Relax, darling, and enjoy a lovely dream where we have each other to ourselves. 
				t ...
				im images/perfume/perfumeHome2-1.gif
				roommate Haah~! Too good~!
				madame Mmm, good dear. Let it flow through you, and express it as passionately as you desire. 
				t roommateF squirms, feeling as though she's being devoured. Each glance of the madameF's fingertips, each hot sigh of the madameF's breaths, each cool electric sensation of the madameF's touch on her flesh, all of these cause her heart to beat like a drum. 
				t ...
				im images/perfume/perfumeHome2-1.gif
				roommate Hah~!
				madame Oh, darling, do not be surprised. My entire body is my weapon to inflict pleasure. 
				t The two intertwined, one the face of grace and poise, the other a hungering mess of a woman, dance together in a lewd frenzy of skin. Each gasp, twitch, and shake that comes from roommateF serves as fuel for the madameF's amusement.
				t ...
				t And in the end roommateF is left finished, completely worn out and asleep while the madameF gently dabs at the sweat on her own body with a decorated cloth.
				player So what was that bottle?
				madame Hmhm~ What indeed? While you carry my vessel, the fluid within is merely a compound that allows one to perceive the hidden. <br>So, the woman has enjoyed herself. I can tell you desire her body, yet you watched with amusement as be became one.
				player I can respect talent when I see it.
				madame Oh? My my, I must say. I hadn't expected to meet someone who resembles my reflection quite so much. For the first time in quite a while, I'm glad to be awake. Thank you for all of this, scholar.
				t The madameF seems quite satisfied, both with you and the state of the world she's awoken into.
			`);
			data.player.perfumeHappy = 5;
			break;
		}
		case "perfumeOutdoor1": {
			writeHTML(`
				t It doesn't take long for madameF to find herself a target. Sitting across a man is gymF, who works at a local gym. She's absentmindedly nodding along with whatever he's saying until the madameF approaches her from behind.
				t gymF looks around, confused.
				madame Relax yourself. This is merely a fantasy, one borne of a thirst your partner can't meet.
				im images/perfume/perfumeOutdoor1-1.gif
				t gymF can't help but let out a small squeal in the heat of the moment.
				sp ???; im images/none.png; What was that?
				gym Nothing! E-everything's fine~! 
				t The madameF's movements are unrestricted, not pausing to consider her target's preferences, almost like her instincts allow her to take for granted that her prey will happily welcome the treatment.
				gym Ghhfff...
				t Which gymF does, biting her knuckle to keep her voice under control. Her eyes are half-lidded as her lower-half becomes a sopping mess beneath the madameF's touch.
				im images/perfume/perfumeOutdoor1-2.gif
				t And before your eyes the nervous twitching in gymF's body ends, she seems almost to melt into her chair like relief from an unbearable heat has arrived.
				t Only just now has the boyfriend realized gymF's state, her face matted with sweat,  cheeks flushed, her whole body rising with each panted breath.
				gym I... I think I might be bi...
				t And on that note the two of you leave, madameF looking quite smug.
				t The madame seems a little more satisfied! 
			`);
			data.player.perfumeHappy += 1;
			break;
		...
		case "perfumeSister1": {
			writeHTML(`
				player Alright, I'm gonna take a breather. You're free to wander around the place, just try not to cause too much damage.
				madame Oho~<br>I appreciate the trust, but scholar, isn't this just your way of testing what I get up to when the cat's away?
				player You've got an opportunity to relax, take it or leave it.
				t You mumble at her as you lay down on the couch, only a little grumpy she saw through your plan.
				t ...
				t You wake up, feeling groggy. The house is still intact, and she didn't even scribble anything on your face while you were asleep. 
				t That said, you hear moaning and other lewd noises from a nearby room. It's loud, and there's music blaring over it too. The sounds are messy, and overlapping. She must've brought over at least twenty people...
				t You peer into sisterF's room to see her sitting at her computer desk with the madameF looking over her shoulder. All of her monitors, even her phone, all the screens are displaying...
				im images/perfume/perfumeSister1-1.gif
				im images/perfume/perfumeSister1-2.gif
				im images/perfume/perfumeSister1-3.gif
				madame My my, it's quite the feast, isn't it?
				sister I shouldn't... I don't even... Even...
				t It seems like sisterF doesn't realize the madameF is guiding her actions. 
				madame I must say, I'm impressed with the state of the world today. To think one could match the feeling of being watched by an entire crowd, all from a nearly empty home... What do you think, child?
				im images/perfume/perfumeSister1-4.gif
				t She asks, retorically, as sisterF can't bring herself to look away. That's mostly because the madameF has her hand on sisterF's chin, but still.
				t There's no telling how long they've been at it, the madameF toying with sisterF while a visual and auditory barrage of lesbian sex assaults her brain.
				t It's probably best to finish here, so you step out and give sisterF's door a good knock.
				player What's going on in there?
				sister Aaah!
				t The noises suddenly stop one after the other, you can actually hear her disconnecting the cables to her pc in a frenzy.
				sister Nothing! Uh, pop-ups! Ads! Haha!
				t And while sisterF scrambles, the madameF comes out of the room with a bemused expression.
				t The madame seems a little more satisfied! 
			`);
			data.player.perfumeHappy += 1;
			break;
		...
		case "perfumeDream1": {
			writeHTML(`
				t The cold of the night nips at your skin. Something rough and grainy is beneath your feet but you can't make it out in the darkness. It feels like you're nowhere.
				madame You're overreacting, you know. Your wife merely understands her true desires now, shouldn't you be happy for her?
				t The bewitching demon speaks to you, but you must resist. The crone has already fooled you once, and the sanctity of your marriage has suffered for it.
				madame What exactly is your plan? Throw me into the sands and die of thirst on your way home? I'll merely sleep until someone else finds me, this is hardly even an inconvenience.
				t Your muscles burn, even against the cold of the desert night. You can still see the image of when you walked into your own home, to find the woman sworn to you in bed with...
				im images/perfume/perfumeDream1.gif
				t With the demon you were cursed to discover.
				t Rage fueling you, you cast the bottle out into the desert, hoping it shatters when it lands.
				madame Hah, hopefully I'm found by someone less boring nex-
				t And she is gone without a trace, her accursed beauty no longer to befoul your home. 
			`);
			break;
		}
		case "cageResearch1": {
		...
		case "handMirrorResearch1": {
			tempScene = "vault";
			writeHTML(`
				t The mirror's reflection becomes cloudy, before the fog fades and you're left with the vision of roommateF.
				t ...
				t roommateF sighs as she puts away her workout gear. She's felt strange lately, feeling bloated and her mentally tired despite her body feeling full of energy.
				roommate Mmmgh... Maybe I shouldn't have pushed myself. Or maybe I drank too much coffee today? Either way, I need a treat.
				t roommateF reaches into her fridge and pulls out a bottle of soda. As she undoes the top her eyes flutter and her vision fades for a moment.
				t Without realizing it she bumps against the fridge.
				roommate W-whoa, I think I fainted for a second there. Wait, why's this already empty?
				t The once-full bottle is completely drained, and a sweet taste is on roommateF's tongue. The strange feeling has only intensified. She tosses it away, finding her sense of balance to be quickly shot. She barely makes it to her couch.
				t Sweat runs down her forehead and her breathing is ragged. She should be cooling down, be she only feels warmer. Her legs spread on their own, her pose much wider than normal.
				t A sudden pressure builds in her abdomen, and begins to travel down. She tightens her core instinctively trying to relieve the knot of energy inside her, the exertion is enough that her hearing wavers, picking up only her exhausted breathing, racing heartbeat, and a faint wet sound.
				t Her mind entirely absent, her hand moves of its own accord to her soaked groin. She dimly registers she'll need to clean up, but what she feels against her fingers leads to confusion, and then an alarm finally strong enough to shock her out of her haze.
				im images/handMirror/mirrorHome1-1.gif
				t Wordless surprise, complete confusion, and then a burning lust enough to make even her ears hot.
				t Each soft touch and inquisitive rub is felt sharply upon her new appendage, leading to bolder and bolder strokes.
				roommate I have to... Gh, go to the hospital... O-or get some help... I have to... I have to...
				t ...
				im images/handMirror/mirrorHome1-2.gif
				t She can't leave until she calms herself down.
				t She can't properly assess the situation through her clothes.
				t She can't think unless she satisfies her sudden urges.
				t Over and over again excuse after excuse flood her mind, each one more feverish than the last in desperate hope that the sudden hunger overtaking her will be satisfied as soon as possible. The only thing that gives her any pause is a mental flash of girlfriendF's face.
				roommate I... I can't let her...
				t The thought of a fate alone suddenly fills her mind. Suddenly and forcefully her thoughts of excusing her behavior and her panicked worries are entirely focused on how to keep girlfriendF close despite this sudden change to her body. 
				im images/handMirror/mirrorHome1-3.gif
				t And an answer is soon found.
			`);
			break;
		}
		case "handMirrorResearch2": {
			tempScene = "vault";
			writeHTML(`
				t The mirror's reflection becomes cloudy, before the fog fades and you're left with the vision of sisterF.
				t ...
				sister No! NO! 
				im images/handMirror/mirrorSister1-1.gif
				sister NO NO NO!<br>Go away, go away! 
				t Shock was followed by stunned disbelief, then a panicked rejection of reality as sisterF's sanity began to kick in. 
				t She'd been feeling strange, but she thought she'd just been under the weather.
				sister No! Nonono, how can 911 be busy?! I... I don't...
				t Her thoughts spiraling out of control, she calls the first person to come to mind, only to hear a prerecorded message saying to leave a message.
				sister assistantF, please, please call me as soon as you can! Something's happening to me, and-<br>Ghhhhggg~!
				t A pulsing feeling spreads through her body, centered at her crotch. Trying to steel herself, she moves the pillow aside and sees the cock... <b>HER</b> cock practically pulsing in sync with her heart.
				sister It's g-growing, I can't make it stop! A-and I feel like... Like something's... Nggh~!<br>I grew.. A penis, and balls, and... Ngggh~! It feels like I'm gonna squirt! I can't... I can't hold it back! assistantF, I... I need h-
				im images/handMirror/mirrorSister1-2.gif
				sister GHHHHHOU~! GHUMMMING~!
			`);
			break;
		}
		case "handMirrorResearch3": {
			tempScene = "vault";
			writeHTML(`
				t bossL strides through a hallway connected to several quarantine cells.
				agent Most cases are the same. Dick growth for women, feminization for men, kind of like a dickgirl zombification process.
				t bossL walks past another cell but hesitates.
				im images/handMirror/mirrorWork1-1.gif
				agent Some of them are downright inhuman. Stamina, resilience-
				boss Length, thickness...
				agent ... Yes, those too. Their sperm is highly infectious, just one drop would be enough. Other bodily fluids seem to have the same effect, albeit acting more slowly.
				t bossL attempts to resume her walk, only to stop and gawk at the very next cell.
				im images/handMirror/mirrorWork1-2.gif
				agent As far as we know the infection started in several locations foreign agents from across the alternate reality mirror visited.<br>There's been talk that... Since the infections mostly started in and around the vault...
				t agentF falters as her phone vibrates, still oblivious to the look bossL has in her eyes.
				agent Oh god... Ma'am, the vault's been designated as fully breached. Evacuation orders have been cancelled, the city's been quarantined...
				boss ... I see.<br>Agent agentL, good work. You're hereby relieved.
				agent This is it, isn't it? What do we do?<br>Ma'am? Why are you undressing?
				boss I'm disabling lockdown on these rooms in a few minutes, agentF. You joining me?
				agent I... Well, there are worse ways to go...
			`);
			break;
		}
		case "charityResearch1": {
			tempScene = "vault";
			writeHTML(`
				t assistantF holds the cage in her hand with trepidation. When she took it the cage became a simple metal one with a wire-frame like design meant to expose the skin.
				assistant Are... Are you sure about this?
				player It's for science, assistantF. Relax. Besides, it's not like you can wear it.
				assistant Right... Science. Okay, hold still...
				im images/cage/cage1.gif
				t You shiver a little as the cold metal cage encapsulates your member. The moment the lock clicks into place, that shiver suddenly travels through your body and you feel... Confined.
				assistant Okay, it's on.
				t Her breathing is slower and deeper than before. If her mouth were open she'd absolutely be drooling right now.
				player It's affecting you too?
				assistant Maybe. I think so. So for testing we need to actually stimulate you, right?<br>I have... I have a few things we could use...
				t ...
				im images/cage/cage2-1.gif
				t You can't place exactly what the cage is doing to you, but your libido is absolutely on <b>fire</b>.
				assistant Hold it in just a little longer, okay?
				player I can't...
				assistant You can, come on...
				t Her body language has totally changed, it's like she's hypnotized. 
				player I... Nggh!
				im images/cage/cage2-2.gif
				assistant Squirt... Squirt... Squirt...
				t Her legs are quivering atop you with each squirt, like she's drawing pleasure from the torment.
				t You grit your teeth, your cock desperately straining at the metal cage while your orgasm leaks out of you.
				t And once it's finished the room is quiet except for your and assistantF's panting.
				player Hff... It's over... Any... Any effects so far?
				assistant ...
				t ...
				im images/cage/Cage3.gif
				assistant One more... Just one more...
				player I can't! I cuh... GHH~
				assistant Ahah... Ahahah~! More~!
				t Your vision begins to swim as you feel weaker with every spurt from your caged penis. It's not the normal sort of post-orgasm exhaustion, you're absolutely sure you're losing something. That something is being stolen. Darkness creeps around you, but you swear there's a flash of light just before you pass out, and the last thing you hear is assistantF suddenly gasping in wordless pleasure.
				t ...
				t You wake up slowly, the light of the room is blinding at first. You're tired, disorientated, but at least you're not caged up anymore.
				assistant You're awake!
				player Yeah... What happened?
				assistant We used the cage. You passed out at the end, and I...
				t She fidgets with her skirt for a moment nervously.
				assistant I grew a, um... Well, the effects were temporary! And you're back to normal too, so everything's fine!
				t You rub your temple, too tired to inquire further.
			`);
			break;
		}
		case "charityResearch2": {
			tempScene = "vault";
			writeHTML(`
				assistant Another session, okay. Hoo, alright!
				player You seem excited.
				assistant I do? Er, I guess... Okay, hold still!
				im images/cage/cage1.gif
				t assistantF slips on the cage once more, it almost seems like it goes on a little easier this time. The moment it clicks into place this time though, the effects are immediate. You fall to your knees as a broiling heat floods through your body, followed by a sudden chill.
				assistant OOOH~!
				t assistant must be experiencing something similar as she clutches at the crotch and goes bow-legged. She grits her teeth and her expression is a dopey smile as you fall to the ground.
				t You feel much weaker as you lay on the floor, like your strength has been sapped away. You take a couple of deep breaths and roll over as you hear clothes hitting the floor.
				assistant Ehe... Ehehe~
				t You feel assistantF grab you by the legs and spread them apart. As you look down you can see what caused her earlier reaction.
				im images/cage/Cage5.gif
				t She giggles like a lunatic as she rubs her brand-new cock against your caged one. Each time her skin touches yours it feels like you're being hit with lightning.
				player assistantF? M-Maybe we should stop here?
				assistant Hehe... First you, then bossL... Then...
				t She's completely lost it. And to make matters worse you're starting to lose it too. A cold sweat is breaking out across your body as you watch her dick sway from side to side. Her balls twitch and a leak of precum jets out onto your dick and you forcibly take in a breath through grit teeth.
				t As if in some sort of defense, or as a means of submission, your dick begins to leak, squirting onto her superior member. You instinctively understand now, this isn't some temporary effect anymore, this is an irreversible act of size theft. And with every second that passes and squirt you shoot out, something else is being stolen as well.
				t ...
				assistant Fuck yeah... Shake it...
				im images/cage/Cage6.gif
				t Your willpower, your resistance, all faded away by now. All of your strength has gone to assistantF now as she sits back jerking herself off to your show. Eyefucking you, molesting you, doing everything short of giving you what you need.
				t As her own balls contract and her cock begins to leak an orgasm down the length of her shaft, it's clear her mind has begun to plan exactly how she intends to use her new tool. Fucking bossL over you, going to your place to fuck both roommateF and girlfriendF at once, just the thought of what she'll put you through from here on serving to strengthen the force of her orgasm as her sperm jets over your ass.
			`);
			writeText("BAD END");
			writeText("...");
			writeText("But there's a ray of hope. You can go on, and awaken from this bad dream, if you like.");
		...
		case "dollResearch4": {
			...
			writeHTML(`
				...
				sp roommate; Okay, okay, I'm stuck. But I'm still breathing. Will I starve? No, somebody will find me... Let's see, office hours start in about three hours, so- <b>Not soon enough</b>.
		...
		case "dollResearch5": {
			...
			writeHTML(`
				...
				t You fall sideways, but there's no pain. Your eyes are open, but there's no way to look around. A few minutes pass before you hear the door open.
	...
	switch (tempScene) {
								...
								if (data.player.color.includes('pass') == true) {
									...
									if (data.player.color.includes('charm') == true) {
										...
										if (data.player.color.includes('perfume') == true) {
											writeArtifact("perfume");
										}
										else {
											if (timeSince != 1) {
												writeSpeech("assistant", "", "Just in, it's some kinda fluid container. It's waiting for you in the testing chamber!");
												writeArtifact("perfume");
		...
		case "home": {
			...
			switch (data.player.artifact2) {
				...
				case "perfume": {
					if (galleryCheck('perfumeHome1') == false) {
						writeFunction("writeEvent('perfumeHome1')", "Show the madameF around the house");
					}
					else {
						if (galleryCheck('perfumeHome2') == false) {
							writeFunction("writeEvent('perfumeHome2')", "Let the madameF interact with roommateF");
						}
					}
					if (galleryCheck('perfumeOutdoor1') == false) {
						writeFunction("writeEvent('perfumeOutdoor1')", "Take the madameF out on the town");
					}
					if (data.player.color.includes('katyaIntro') == true) {
						if (galleryCheck('perfumeSister1') == false) {
							writeFunction("writeEvent('perfumeSister1')", "Take a nap and allow the madameF to do as she pleases");
						}
					}
					break;
				}
